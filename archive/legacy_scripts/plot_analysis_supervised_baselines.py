"""
Legacy supervised-baseline analysis plots for the MetroPT3 VAE project.

This script is archived because its Logistic Regression and XGBoost baselines
use failure labels for training. The final report-ready fair baseline is the
unsupervised Isolation Forest pipeline in `scripts/train_isolation_forest.py`
and `scripts/compare_baselines.py`.
...
Run:
    python archive/legacy_scripts/plot_analysis_supervised_baselines.py \
      [--run-dir models/vae_runs/<id>]

Defaults to the primary best run: models/vae_runs/20260424_140621

Optional extra dependency: xgboost.
"""
